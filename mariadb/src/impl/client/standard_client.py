...
class Client:
    ...
    def __init__(self, configuration: Configuration, host_address: HostAddress):
        """
    ...
    def _create_socket(self) -> None:
        ...
        try:
            ...
            # Create reader and writer (compression will be enabled after handshake if negotiated)
            socket_stream = SocketStream(self.socket)
            self.reader = PacketReader(stream=socket_stream, debug=self.configuration.debug)
            self.writer = PacketWriter(stream=socket_stream, debug=self.configuration.debug)
    ...
    def _parse_handshake(self, packet: bytes) -> Context:
        ...
        salt_length = 0
        if (server_capabilities_4_first_bytes & CAPABILITY.PLUGIN_AUTH) != 0:
            if pos < len(packet):
        ...
        seed = seed1
        if (server_capabilities_4_first_bytes & CAPABILITY.SECURE_CONNECTION) != 0:
            if salt_length > 0:
        ...
        # Since MariaDB 10.2
        if (server_capabilities_4_first_bytes & CAPABILITY.MYSQL) == 0:
        ...
        if self.configuration.init_command:
            try:
                ...
                if init_command:
                    ...
                    self.execute(query_packet, can_redo=False)                   
    ...
    def _enable_compression_if_negotiated(self) -> None:
        ...
        if (self.configuration.use_compression and 
            self.context and 
            self.context.has_capability(CAPABILITY.COMPRESS)):
                        
            # Create compression stream wrapper
            compress_stream = CompressStream(socket=self.socket, debug=self.configuration.debug)
    ...
    def _parse_ok_packet(self, packet: bytes) -> Completion:
        ...
        if pos < len(packet):
            ...
            if (self.context and 
                self.context.has_capability(CAPABILITY.SESSION_TRACKING) and 
                pos < len(packet)):
    ...
    def _cleanup_connection(self) -> None:
        ...
        if self.writer:
            try:
                # COM_QUIT packet
                self._send_packet_chunk(bytearray([0x01]), "COM_QUIT")
                self.writer.close()
            except:
                pass
            self.writer = None
