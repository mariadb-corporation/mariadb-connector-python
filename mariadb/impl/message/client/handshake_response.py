...
class HandshakeResponse(ClientMessage):
    ...
    def payload(self, context: Context, writer: PayloadWriter) -> bytearray:
        ...
        auth_response = NativePasswordPlugin.encrypt_password(self.configuration.password, context.auth_data)  # type: ignore[arg-type]
        if context.server_capabilities & CAPABILITY.PLUGIN_AUTH_LENENC_CLIENT_DATA:
            stream.write_length_encoded_int(len(auth_response))
            stream.write_bytes(auth_response)
        elif context.server_capabilities & CAPABILITY.SECURE_CONNECTION:
