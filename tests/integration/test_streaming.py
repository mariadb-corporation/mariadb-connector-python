...
class TestStreaming(unittest.TestCase):
    ...
    def test_streaming_multi_resultset(self):
        con = mariadb.connect(**conf())
        setup = con.cursor()
        setup.execute("DROP PROCEDURE IF EXISTS p_sync_stream_multi")
        setup.execute("CREATE PROCEDURE p_sync_stream_multi()\n"
                      "BEGIN\n  SELECT 100 AS a;\n  SELECT 200 AS b;\nEND")
        setup.close()
        try:
            ...
            cleanup = con.cursor()
            cleanup.execute("DROP PROCEDURE IF EXISTS p_sync_stream_multi")
            cleanup.close()
            con.close()

    def test_close_mid_multi_resultset(self):
        con = mariadb.connect(**conf())
        setup = con.cursor()
        setup.execute("DROP PROCEDURE IF EXISTS p_sync_close_multi")
        setup.execute("CREATE PROCEDURE p_sync_close_multi()\n"
        ...
        try:
            ...
            cleanup = con.cursor()
            cleanup.execute("DROP PROCEDURE IF EXISTS p_sync_close_multi")
            cleanup.close()
            con.close()
