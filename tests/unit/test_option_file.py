...
@pytest.fixture
def cnf(tmp_path: Path) -> tuple[str, str]:
...
def test_client_group_mapping(cnf: tuple[str, str]) -> None:
    path, _ = cnf
...
def test_includedir(cnf: tuple[str, str]) -> None:
    path, _ = cnf
...
def test_from_dict_applies_file(cnf: tuple[str, str]) -> None:
    path, _ = cnf
...
def test_explicit_kwargs_override_file(cnf: tuple[str, str]) -> None:
    path, _ = cnf
...
def test_no_default_params_means_no_file_read(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_default_file_none_reads_nothing(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
@skip_on_windows
def test_empty_default_file_scans_search_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    (tmp_path / ".my.cnf").write_text("[client]\nuser = fromhome\n")
    assert Configuration.from_dict({"default_file": ""}).user == "fromhome"


def test_specific_file_skips_search_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    (tmp_path / ".my.cnf").write_text("[client]\nuser = fromhome\n")
...
@skip_on_windows
def test_default_group_alone_scans_search_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))
    (tmp_path / ".my.cnf").write_text("[client]\nuser = fromhome\n")
