...
class CursorMariaDBTest(unittest.TestCase):
  ...
  def tearDown(self):
    ...
    def test_update_parameter(self):
      cursor = self.connection.cursor()
      cursor.execute("CREATE TEMPORARY TABLE test_update_parameter(a int not null auto_increment "
    ...
    def test_delete_parameter(self):
      cursor = self.connection.cursor()
      cursor.execute("CREATE TEMPORARY TABLE test_delete_parameter(a int not null auto_increment "
                     "primary key, b int, c int, d varchar(20),e date)")
      cursor.execute("set @@autocommit=0");
      list_in = []
      for i in range(1, 300001):
